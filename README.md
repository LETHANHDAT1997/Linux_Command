...
| **`/dev`** | **Chứa các tệp thiết bị đại diện cho phần cứng (như /dev/sda cho ổ đĩa).** |
...
#### Một số biến môi trường thường gặp khác
- **`HOME`**: Đường dẫn đến thư mục chính của người dùng hiện tại.
- **`USER`**: Tên người dùng hiện tại.
- **`SHELL`**: Đường dẫn đến shell mặc định (ví dụ: `/bin/bash`).
- **`PWD`**: Thư mục hiện tại.
- **`LANG`**: Ngôn ngữ và mã hóa mặc định của hệ thống.
- **`EDITOR`**: Trình soạn thảo mặc định khi mở file từ dòng lệnh.
- **`TZ`**: Múi giờ của hệ thống.
- **`HOSTNAME`**: Tên máy của hệ thống.
- **`MAIL`**: Đường dẫn hộp thư của người dùng.

**Ví dụ kiểm tra giá trị biến:**
```bash
echo $HOME
echo $USER
echo $LANG
...
### 🔄 Đăng Nhập, Đăng Xuất và Chuyển Đổi Người Dùng

Các lệnh này cho phép bạn chuyển đổi giữa các tài khoản người dùng khác nhau và quản lý phiên làm việc của mình.

####  `login`

Lệnh `login` được sử dụng để bắt đầu một phiên làm việc mới trên hệ thống. Khi bạn kết nối qua console hoặc terminal, bạn thường sẽ thấy một dấu nhắc đăng nhập yêu cầu tên người dùng và mật khẩu.

```bash
login
# Yêu cầu nhập tên người dùng và sau đó là mật khẩu
```

####  `logout` & `exit`

Để kết thúc phiên làm việc của người dùng hiện tại và quay trở lại màn hình đăng nhập (hoặc đóng cửa sổ terminal), hãy sử dụng lệnh `logout`.

```bash
logout
```

Trong hầu hết các shell hiện đại, lệnh `exit` cũng có tác dụng tương tự là đóng phiên làm việc.
...
Lệnh `su` cho phép bạn chuyển đổi sang một tài khoản người dùng khác ngay trong phiên làm việc hiện tại, miễn là bạn biết mật khẩu của người dùng đó.
...
Hệ thống sẽ yêu cầu bạn nhập mật khẩu của `alice`. Sau khi xác thực thành công, bạn sẽ hoạt động với tư cách là người dùng `alice`.

**Chuyển sang người dùng root:** Đây là cách phổ biến nhất để có được quyền quản trị viên cao nhất.
...
Khi chạy lệnh này, bạn sẽ được yêu cầu nhập mật khẩu của người dùng `root`. Dấu gạch nối (`-`) rất quan trọng vì nó mô phỏng một phiên đăng nhập đầy đủ của người dùng `root`, bao gồm cả việc tải các biến môi trường và chuyển đến thư mục nhà của `root` (`/root`).

**Sự khác biệt giữa `su` và `sudo`:**

  * **`su -`**: Bạn trở thành người dùng `root` và có thể chạy bất kỳ lệnh nào mà không cần `sudo` phía trước, nhưng bạn phải biết mật khẩu của `root`.
  * **`sudo <lệnh>`**: Bạn thực thi một lệnh duy nhất với quyền `root`. Bạn cần nhập mật khẩu của *chính mình* (người dùng hiện tại), và người dùng của bạn phải thuộc nhóm `sudo`. Đây là phương pháp được khuyến nghị vì nó an toàn và dễ kiểm soát hơn.

---
**Ghi chú chung:**
- Các lệnh quản lý người dùng và nhóm thường yêu cầu quyền `sudo`.
- Sau khi thay đổi nhóm hoặc quyền, có thể cần đăng xuất và đăng nhập lại để áp dụng.

### `Ví dụ và mẹo:`

#### Tạo người dùng mới với thư mục nhà và shell cụ thể
```bash
sudo useradd -m -d /custom/home/alice -s /bin/zsh alice
```
- `-d /custom/home/alice`: Đặt thư mục nhà tùy chỉnh.
- `-s /bin/zsh`: Đặt shell mặc định là `zsh`.

#### Thêm người dùng vào nhiều nhóm cùng lúc
```bash
sudo usermod -aG sudo,developers,git alice
```
- `-aG`: Thêm (`append`) vào nhóm, giữ nguyên các nhóm hiện có.

#### Xem thông tin chi tiết của người dùng
```bash
id alice
```
Hiển thị UID, GID, và các nhóm mà `alice` thuộc về.

#### Khóa hoặc mở khóa tài khoản người dùng
```bash
sudo usermod -L bob   # Khóa
sudo usermod -U bob   # Mở khóa
```
Khi bị khóa, người dùng không thể đăng nhập.

#### Đổi mật khẩu người dùng khác (yêu cầu sudo)
```bash
sudo passwd bob
```

#### Tạo nhóm và thêm người dùng vào nhóm ngay khi tạo
```bash
sudo groupadd project_team
sudo useradd -m -G project_team charlie
```

#### Xóa người dùng nhưng giữ lại thư mục nhà
```bash
sudo userdel bob
```
Nếu muốn xóa cả thư mục nhà:
```bash
sudo userdel -r bob
```

#### Liệt kê tất cả người dùng và nhóm trên hệ thống
```bash
cut -d: -f1 /etc/passwd   # Danh sách người dùng
cut -d: -f1 /etc/group    # Danh sách nhóm
```

#### Thay đổi quyền sở hữu file/thư mục cho nhóm mới
```bash
sudo chown :project_team /var/www/html
sudo chmod 770 /var/www/html
```
Điều này cho phép tất cả thành viên của nhóm `project_team` đọc, ghi, và thực thi trong thư mục đó.

**Mẹo:** Sử dụng `groups <username>` để xem một người dùng thuộc nhóm nào.
...
# 📚 Phụ Lục: Lệnh Nâng Cao và Mẹo Sử Dụng

## ⚙️ Quản Lý Dịch Vụ với systemctl
- `systemctl status <service>`: Xem trạng thái dịch vụ.
- `sudo systemctl restart <service>`: Khởi động lại dịch vụ.
- `sudo systemctl enable <service>`: Tự động chạy dịch vụ khi khởi động máy.

## 📦 Nén & Giải Nén Nâng Cao
- `zip -r archive.zip folder/`: Nén thư mục thành file `.zip`.
- `unzip archive.zip`: Giải nén file `.zip`.
- `tar -cJvf archive.tar.xz folder/`: Nén với định dạng XZ (tốt hơn gzip).
- `7z a archive.7z folder/`: Nén với 7-Zip (cần cài `p7zip-full`).

## 📡 Mẹo Mạng
- `ip link set eth0 down` / `ip link set eth0 up`: Tắt / bật interface mạng.
- `curl -I https://example.com`: Chỉ lấy HTTP header.
- `rsync -avz folder/ user@host:/path/`: Đồng bộ dữ liệu nhanh và tiết kiệm băng thông.

## 🛡️ Lệnh Nguy Hiểm (Cần Thận Trọng)
⚠️ Các lệnh dưới đây có thể gây mất dữ liệu hoặc hỏng hệ thống:
- `rm -rf /`
- `rm -rf *` (trong thư mục sai)
- `chmod -R 777 /`

Luôn kiểm tra đường dẫn và quyền trước khi thực thi.

## 💡 Mẹo Hữu Ích
- Dùng `history | grep <từ_khóa>` để tìm lại lệnh đã chạy.
- Dùng `!!` để chạy lại lệnh cuối cùng.
- Dùng `!<số>` để chạy lại lệnh trong history.